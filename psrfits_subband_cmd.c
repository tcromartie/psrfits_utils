...
usage(void)
{
  fprintf(stderr,"%s","   [-dm dm] [-nsub nsub] [-dstime dstime] [-startfile startfile] [-numfiles numfiles] [-outbits outbits] [-edge_chan edge_chan] [-filetime filetime] [-filelen filelen] [-tgtstd tgtstd] [-tgtavg tgtavg] [-bytes] [-onlyI] [-weights wgtsfile] [-o outputbasename] [--] infile ...\n");
  fprintf(stderr,"%s","      \n");
  ...
  fprintf(stderr,"%s","       -tgtstd: Target standard dev will be set in code based on outbits\n");
  fprintf(stderr,"%s","                1 float value between 0.0 and 1000.0\n");
  fprintf(stderr,"%s","                default: `0.0'\n");
  fprintf(stderr,"%s","       -tgtavg: Target average for converted data; set in code based on outbits\n");
  fprintf(stderr,"%s","                1 float value between 0.0 and 1000.0\n");
  ...
  fprintf(stderr,"%s","  version: 15Jun16\n");
  fprintf(stderr,"%s","  ");
  exit(EXIT_FAILURE);
}
/**********************************************************************/
...
parseCmdline(int argc, char **argv)
{
  ...
  for(i=1, cmd.argc=1; i<argc; i++) {
    ...
    if( 0==strcmp("-tgtstd", argv[i]) ) {
      ...
      cmd.tgtstdC = i-keep;
      checkFloatLower("-tgtstd", &cmd.tgtstd, cmd.tgtstdC, 1000.0);
    ...
    if( 0==strcmp("-tgtavg", argv[i]) ) {
      ...
      cmd.tgtavgC = i-keep;
      checkFloatLower("-tgtavg", &cmd.tgtavg, cmd.tgtavgC, 1000.0);
